...
namespace NovoBanco.Application.UseCases.Withdraw;

public class WithdrawHandler
{
    private readonly IApplicationDbContext _context;

    public WithdrawHandler(IApplicationDbContext context)
    {
        _context = context;
    }

    public async Task Handle(WithdrawRequest request, CancellationToken cancellationToken)
    {
        if (request.Amount <= 0)
